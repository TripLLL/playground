...
make.tabs_prediction <- function(fake_data_list){
  ...
  for(i in 1:(nrow(results))){#!!!!
    ...
    'source("Fig_1F.r");',
    'source("Figure_2A-D.r");','source("Checks.R");plot_grid(plot1,plot2,plot3)',
     '\n',
    '```', '\n\n')
  }
  return(res)
...
pairs_tabs <- function(stan_model_list){
  res2 <- NULL
  for(i in 1:nrow(results)){
    res2[i] <- paste0( '###  ', paste0(with(data.frame(t(rbind( names(results[,1:4]),c(results[i,1:4]),rep("\n",4)))), paste0(X1,X2,X3),rep("\n",4)),collapse=" "), " \n",
    "```{r pairs",i,", results = 'asis'}", "\n",
                      'pairs(stan_model_list[[',i,']],pars=colnames(result)[!is.na(result[',i,',])])', "\n",
                      '```', '\n\n#')
  }
  return(res2)
}
